...
> Product source of truth: `ai_job_portal_prd.pdf` (PRD v1.0).
...
`.env.example` lists every variable the project reads, with empty values. Fill them using the [environment variables](#environment-variables) table below. `.env` is git-ignored — never commit real secrets.
...
| `JWT_ACCESS_TOKEN_SECRET`               | yes      | _(random string)_                                     | Signing secret for short-lived access tokens                           |
| `JWT_ACCESS_TOKEN_EXPIRES_IN`           | no       | `15m`                                                 | Access token lifetime (`s`/`m`/`h`/`d`, or bare seconds)               |
| `JWT_REFRESH_TOKEN_SECRET`              | yes      | _(random string)_                                     | Signing secret for refresh tokens — must differ from the access secret |
...
| `REFRESH_TOKEN_ENCRYPTION_KEY`          | yes      | _(base64, 32 bytes)_                                  | AES-256-GCM key protecting refresh tokens at rest                      |
...
| `UPLOAD_URL_SIGNING_SECRET`             | yes      | _(random string)_                                     | HMAC-SHA256 key for signing resume upload URLs                         |
...
Email verification state lives here too: `isEmailVerified` (defaults to `false`), `emailVerifiedAt`, and `passwordChangedAt`. All three are unwritten today — see [Implementation status](#implementation-status).

### `verification_tokens`

Single-use, expiring tokens backing both email verification and password reset, distinguished by the `VerificationTokenType` enum (`email_verification` \| `password_reset`).

| Column        | Notes                                                                             |
| ------------- | --------------------------------------------------------------------------------- |
| `token_hash`  | `CHAR(64)`, unique — the SHA-256 **hex** digest of the token, never the raw value |
| `expires_at`  | Absolute expiry, set from the matching `*_TOKEN_TTL` variable                     |
| `consumed_at` | `NULL` until redeemed; the row is kept, not deleted                               |
| `user_id`     | Cascades on user deletion; indexed with `type` for per-user lookups               |

Three design choices worth knowing before writing against this table:

- **Hashed, not encrypted.** These are bearer credentials, so a database leak must not yield usable tokens. That rules out the reversible `EncryptionService` used for refresh tokens. The raw token exists only in the outbound email; `CHAR(64)` is sized for a hex digest, so the hash must be `digest('hex')` — a base64url digest would be blank-padded by Postgres and never match on lookup.
- **`user_id` is intentionally not unique.** A user can hold an email verification token and a password reset token at the same time, and retained consumed rows would otherwise block every future token for that account.
- **Consumed rows are retained** so the API can distinguish "this link was already used" from "this link never existed" and show the right message. This implies a cleanup job for expired and consumed rows, which does not exist yet.
